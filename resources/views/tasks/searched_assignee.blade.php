@if(Arr::has($searched_users, 'data'))
    @forelse($searched_users['data'] as $searched_user)
        <div class="cursor-pointer d-flex flex-stack py-4 border-bottom border-gray-300 border-bottom-dashed searched_user" data-searched-email="{{$searched_user['user_email']}}" data-searched-name-en="{{$searched_user['user_name_en']}}" data-searched-name-bn="{{$searched_user['user_name_bn']}}" data-user-info="{{json_encode($searched_user, JSON_UNESCAPED_UNICODE)}}">
            <div style="width: 80%" class="d-flex align-items-center">
                <div class="symbol symbol-35px symbol-circle">
                    <img alt="Pic" src="{{asset('assets/media/avatars/blank.png')}}"/>
                </div>
                <div class="ml-5">
                    <a href="javascript:;" class="fs-5 fw-bolder text-dark h5 mb-2">{{$searched_user['user_name_bn']}}</a>
            ...
            <div style="width: 20%" >
                <input type="checkbox" onclick="$(this).parent().parent().click()">
            </div>
        </div>
    @empty
        @if(filter_var($search_key, FILTER_VALIDATE_EMAIL))
            <div class="cursor-pointer d-flex flex-stack py-4 border-bottom border-gray-300 border-bottom-dashed searched_user" data-searched-email="{{$search_key}}" data-searched-name-en="{{explode('@',$search_key)[0]}}" data-searched-name-bn="{{explode('@',$search_key)[0]}}"
...
<script>
    ...
    $('.searched_user').click(function () {
        TaskContainer.addUserInInviteListWithDateRange($(this));
    })
    @else
    $('.searched_user').click(function () {
        TaskContainer.addUserInInviteList($(this));
    })
    @endif
</script>
